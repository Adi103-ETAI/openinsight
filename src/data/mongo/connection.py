...
def get_mongo_client(
    ...
    pool_options = {
        ...
        "serverSelectionTimeoutMS": server_selection_timeout_ms or settings.mongodb_server_selection_timeout_ms,
    }
    ...
    try:
        ...
        # Use sync admin command for connection test
        _mongo_client.admin.command('ping')
        loop.run_until_complete(asyncio.sleep(0))  # Allow async operations to complete
        loop.close()
        logger.info("[MongoPool] Connection pool initialized successfully")
    except Exception as e:
        logger.warning("[MongoPool] Connection test failed: %s", e)
